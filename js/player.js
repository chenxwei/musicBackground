var Ajax = {
    get: function (url, fn) {
        ...
        xhr.open('GET', url, false); //异步true，同步false
...
let url = 'http://staticapi.techtimesun.com/VrMusic/music_config.json';
Ajax.get(url, getJSon); //请求json
let musicList = []; //音乐列表
...
async function getJSon(result) {
    let list = [...result.musicList];
    await list.forEach(function (item) {
        item.title = item.name;
        item.singer = 'Campsite Dream';
        item.songUrl = `http://staticapi.techtimesun.com/VrMusic/${item.id}.mp3`;
        item.imageUrl = './images/songs/changdie.jpg';
    })
    musicList = [...list];
    if(musicList.length===0){
        alert('暂无歌曲！')
    }
    let queryMp3 = getQueryString('mp3');
    let defaultIndex = -1;
    if(queryMp3){
        defaultIndex = musicList.findIndex(item=>item.id === queryMp3);
    }
    //创建一个音乐播放器的类 单例模式
    class Player {
        constructor() { //类的构造函数
    ...
    class PlayerCreator {
        constructor() {
            ...
            this.song_index = defaultIndex==-1?0:defaultIndex; // 当前播放的歌曲索引
        ...
        bindEventListener() {
            ...
            if(defaultIndex !==-1){
                $(".player-control__btn--play").click();
            }
